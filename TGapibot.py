...
<b>🚀 {self.t(user_id, "help_main_features")}</b>
• {self.t(user_id, "help_proxy_auto_detect")}
• {self.t(user_id, "help_realtime_progress")}
• {self.t(user_id, "help_session_tdata_support")}

<b>📁 {self.t(user_id, "help_supported_formats")}</b>
• Session + JSON {self.t(user_id, "help_files")}
• TData {self.t(user_id, "help_folders")}
• ZIP {self.t(user_id, "help_archives")}

<b>🔄 {self.t(user_id, "help_format_conversion")}</b>
• Tdata → Session
• Session → Tdata
• {self.t(user_id, "help_batch_processing")}

<b>📡 {self.t(user_id, "help_proxy_features")}</b>
• {self.t(user_id, "help_auto_read_proxy")}
• {self.t(user_id, "help_proxy_types")}
        """
        
        if self.db.is_admin(user_id):
            admin_cmds = self.t(user_id, "help_admin_commands")
            speed_opt = self.t(user_id, "help_speed_optimization")
            help_text += f"""

<b>👑 {admin_cmds}</b>
• /addadmin [ID/{self.t(user_id, "help_username")}] - {self.t(user_id, "help_add_admin")}
• /removeadmin [ID] - {self.t(user_id, "help_remove_admin")}
• /listadmins - {self.t(user_id, "help_list_admins")}
• /proxy - {self.t(user_id, "help_proxy_status")}
• /testproxy - {self.t(user_id, "help_test_proxy")}
• /cleanproxy - {self.t(user_id, "help_clean_proxy")}

<b>⚡ {speed_opt}</b>
• {self.t(user_id, "help_fast_mode")}: {config.PROXY_FAST_MODE}
• {self.t(user_id, "help_concurrent_checks")}: {config.PROXY_CHECK_CONCURRENT}
• {self.t(user_id, "help_smart_retry")}: {config.PROXY_RETRY_COUNT}
            """
...
💡 现在可以开始使用优化后的代理进行账号检测了！
    ...
    def convert_command(self, update: Update, context: CallbackContext):
        ...
        if not OPENTELE_AVAILABLE:
            error_msg = self.t(user_id, "convert_feature_unavailable")
            self.safe_send_message(update, error_msg)
            return
...
<b>📁 {self.t(user_id, "convert_supported_conversions")}</b>
1️⃣ <b>Tdata → Session</b>
   • {self.t(user_id, "convert_tdata_to_session_desc")}

2️⃣ <b>Session → Tdata</b>
   • {self.t(user_id, "convert_session_to_tdata_desc")}

<b>⚡ {self.t(user_id, "convert_features")}</b>
• {self.t(user_id, "convert_batch_concurrent")}
• {self.t(user_id, "convert_realtime_progress")}

<b>📤 {self.t(user_id, "convert_operation_guide")}</b>
{select_prompt}
        """
